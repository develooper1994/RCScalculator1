function tline = seperateLine(charArray) %#codegen
%SEPERATEDLINE
...
% remain = charArray;
tline = ''; %strings codegen not supported, sad but true :'(
LF = 0; CR = 0; Nul = 0; space =0;
lt = 0;
% tline = tline(1:end-sum(lt));
while (~isempty(charArray))
% while (charArray ~= "") %"" olmasý gerekebilir.
    [token,charArray] = strtok(charArray, char(10)); %newline codegen not supported
